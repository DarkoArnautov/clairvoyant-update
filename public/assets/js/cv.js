(function($) {
    ...
    $(document).ready(function() {
        ...
        var revoSaas = $('.revo-slider-cv');
        if (revoSaas.length > 0) {
            $('.revo-slider-cv').show().revolution({
                sliderType: "hero",
        ...
        var $hasSubmenu = $('.menu-item-has-children > a > .arrow, .has-mega-menu > a > .arrow, .has-mega-menu > a > .cv-mobile-menu');
        ...
        function pbOnScrollAnimation(items, trigger) {
            items.each(function() {
                ...
                pbTrigger.waypoint(function(direction) {
                    pbElement.find('.progress-bar > .bar').css({
                        'width': curVal + '%'
                    });
                    pbElement.find('.progress-bar > .value').addClass('is-visible').css({
                        'width': curVal + '%'
                    });
                }, {
                    offset: '88%'
...
/* --------------------------------------------------------------- 
Added JS for active link on mobile.
This code can be optimized after the correct folder structure
----------------------------------------------------------------- */
var sideMenu = document.querySelector('.vertical-navigation').querySelectorAll('a span.cv-mobile-menu');
//console.log(sideMenu);

(function addActive() {
    var link = window.location.pathname;
        
    sideMenu.forEach(anchor => {
        var currentlink = anchor.innerHTML.toLowerCase();
        if(link.indexOf("index") > -1) {
            sideMenu[0].classList.add("current-active-link");
        } 
        else if(link.indexOf(currentlink) > -1) {
            sideMenu[0].classList.remove("current-active-link");
            anchor.classList.add("current-active-link");
        } 
        else if(link.indexOf("confluence") > -1 || link.indexOf("resources") > -1) {
            sideMenu[0].classList.remove("current-active-link");
            sideMenu[3].classList.add("current-active-link");
        }
        else if(link.indexOf("announcement") > -1 || link.indexOf("awards") > -1 || link.indexOf("careers") > -1 || link.indexOf("company") > -1) {
            sideMenu[0].classList.remove("current-active-link");
            sideMenu[7].classList.add("current-active-link");
        }
    })
}());